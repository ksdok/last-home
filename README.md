...
- ✅ **LH-22/LH-23** were intermediate Brita steps; **LH-26** is merged to `main` AND **validated in-game** (2-player MP, `elementary_school`): the embedded role set is replaced with the **22-role Brita PZRolePlay** roster, `vanilla` is added as the no-op hard mode, and required Brita/Arsenal dependencies are restored in `mod.info` (`require=Brita,Brita_2,Arsenal(26)GunFighter[MAIN MOD 2.0]`, comma-separated)
