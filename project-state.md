...
- 🧪 **LH-22** code integrated: `mod.info` now requires `Brita`, `Brita_2`, and `Arsenal(26)GunFighter[MAIN MOD 2.0]`; `LastHomeRoles.lua` adds the **007 Agent** role, increments the picker to 18 roles, and injects `Base.Screwdriver` into every applicable role; `equipRoleItems()` now supports non-back worn containers such as `Base.Bag_Tactical_Belt_Front`. In-game Brita validation remains pending (priming, suppressor mount, clothing IDs).
...
- [x] LH-22 — Rôle 007 Agent + dépendance Brita + tournevis pour tous. Files: `LastHomeRoles.lua`, `LastHomeShared.lua`, `mod.info`, `README.md`, `project-state.md`, `specs/LH-22-agent-brita-tournevis.md`. Added the 18th role **007 Agent** (PPK + `MP5SD6_Fixed`, tactical belt, Brita clothing), added `Base.Screwdriver` to every applicable existing role, required `Brita` / `Brita_2` / `Arsenal(26)GunFighter[MAIN MOD 2.0]` in `mod.info`, and generalized `equipRoleItems()` so non-back worn containers can be equipped from `equipped.bag`. Pending in-game validation for Brita/Arsenal priming and suppressor mounting. `version=0.12.0`.
