...
- Source branch for delivered ticket: `feat/lh-26-roles-pzroleplay` (LH-26: 22-role PZRolePlay Brita port + LH-28 fog spec) — merged to `main` (fast-forward, `4a51773..731e756`)
...
- 🧪 **LH-22/LH-23** remain the validated intermediate Brita steps (007 Agent + 10-role relook), now superseded by **LH-26**.
- ✅ **LH-26** merged to `main` AND **validated in-game** (MP 2 joueurs, `elementary_school`) : rôles Brita appliqués, armes/armures présentes, rôles fonctionnent. Commits: `f7af043`, `1d03f3b`, `731e756`. Note : le `require=Brita,Brita_2,Arsenal(26)GunFighter[MAIN MOD 2.0]` (séparateur **virgule**) est nécessaire — sans Brita/Arsenal activés, `ItemContainer.AddItem: can't find Base.M249` etc. échouait silencieusement à l'attribution (corrigé via `require=` + séparateur virgule, commits `c95a5c4`, `8688570`).
- ✅ **LH-03 / LH-10 / LH-24 (vagues)** validé en jeu (MP) : vagues, scaling, directions, déclenchement manuel vague 1 via K, skip via K, spawn des joueurs au bon endroit.
...
- [x] LH-26 — Reprise des rôles de PZRolePlay dans Last Home (set Brita, 22 rôles) — spec écrite, **implémentation mergée sur `main` et validée en jeu (MP 2 joueurs)**
- [x] LH-28 — Brouillard épais permanent à l'extérieur — **spec écrite, non implémentée**
...
- 🧪 LH-26 — Port PZRolePlay 22 rôles Brita. Files: `LastHomeRoles.lua`, `LastHomeShared.lua`, `LastHomeClient.lua`, `LastHomeRolePicker.lua`, `LastHomeServer.lua`, `LastHomeBoundary.lua`, `mod.info`, `README.md`, `project-state.md`, `specs/LH-26-roles-pzroleplay.md`, `scripts/verify_role_ammo_coverage.py`. Remplace les defs embarquées par le set Brita PZRolePlay (22 rôles + `vanilla` no-op), `applyCarryProfile` suit le modèle PZRolePlay (unlimited carry sauf `vanilla`), `civil` migré en `vanilla`, helpers alignés. **Mergé sur `main` (fast-forward `4a51773..731e756`) ; validation en jeu en attente** (priming armes, accessoires, rigs, vêtements). Commits: `f7af043`, `1d03f3b`. Note : la dépendance `require=Brita,Brita_2,Arsenal(26)GunFighter[MAIN MOD 2.0]` (décision D5 de LH-26 ; **virgule** comme séparateur `mod.info`, pas le `;` du `Mods=` serveur) avait été brièvement retirée du `mod.info` sur demande de l'host, puis **rétablie** après constat en jeu (`~/Zomboid/console.txt`) que sans Brita/Arsenal activés, `ItemContainer.AddItem: can't find Base.M249` / `Base.Armor_Defender` / etc. échouait silencieusement à l'attribution des rôles Brita — les items Brita n'existent dans le moteur que si ces mods sont chargés. Le `require=` garantit un échec de chargement clair (« mod manquant ») plutôt qu'un échec runtime silencieux.
...
- [ ] LH-28 — Brouillard épais permanent à l'extérieur pendant la partie Last Home (prep + vague). Spec : `specs/LH-28-brouillard-epais-exterieur.md`. Override périodique du fog intensity via `ClimateManager`, restreint aux parties scénario (`isScenarioHouse()`), relâché au game over / reset, intérieur du bâtiment scénario préservé. Constantes `FOG_ENABLED` / `FOG_INTENSITY` / `FOG_REFRESH_INTERVAL_SECONDS`. API B41 `ClimateManager` à valider (serveur vs client, setter d'override, rendu intérieur) avant implémentation.
- [x] LH-26 — Valider en jeu le port PZRolePlay (post-merge) : rôles Brita appliqués, armes/armures présentes, rôles fonctionnent — **validé en jeu (MP 2 joueurs, `elementary_school`)**
