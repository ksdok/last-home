...
- ✅ **LH-02** through **LH-08**, **LH-10**, **LH-13**, **LH-14**, **LH-19**, **LH-20** and **LH-21** implemented (**LH-15** removed by **LH-21**)
...
- 🧪 **LH-22** code integrated: `LastHomeRoles.lua` adds the **007 Agent** role, increments the picker to 18 roles, and injects `Base.Screwdriver` into every applicable role; `equipRoleItems()` now supports non-back worn containers such as `Base.Bag_Tactical_Belt_Front`. `mod.info` does **not** currently enforce Brita/Arsenal dependencies in this branch. In-game Brita validation remains pending (priming, suppressor mount, clothing IDs).
- 🧪 **LH-23** code integrated: `LastHomeRoles.lua` rewrites the 10 selected roles with their validated Brita/Arsenal loadouts (weapons, armor, bags, accessories), and `LastHomeShared.lua` now enables `setUnlimitedCarry` for `rambo` and `samourai`. In-game validation remains pending (attachments, priming, Tail/Nose wear slots, overall balance).
...
- [ ] LH-24 — Déclenchement manuel de la 1ère vague (pas de timer d'auto-start)
- [ ] LH-25 — Mod séparé : choix de rôle seul (role picker standalone)
...
- [x] LH-13 — Continuous vanilla/story spawn suppression. Files: `LastHomeWaves.lua`, `specs/LH-13`. `clearAmbientZombiesNearHouse(reason)` with prep/wave/periodic reasons; immediate cleanup at phase transitions; periodic cleanup every 5s while `started` + `house` set in Challenge mode; `LH_waveZombie` zombies excluded; `Server.nextAmbientCleanupAt` scheduled + reset in `resetState()`.
...
- [x] LH-22 — Rôle 007 Agent + dépendance Brita + tournevis pour tous. Files: `LastHomeRoles.lua`, `LastHomeShared.lua`, `mod.info`, `README.md`, `project-state.md`, `specs/LH-22-agent-brita-tournevis.md`. Added the 18th role **007 Agent** (PPK + `MP5SD6_Fixed`, tactical belt, Brita clothing), added `Base.Screwdriver` to every applicable existing role, and generalized `equipRoleItems()` so non-back worn containers can be equipped from `equipped.bag`. Note: despite the original LH-22 intent, `mod.info` in this branch does **not** currently enforce `Brita` / `Brita_2` / `Arsenal(26)GunFighter[MAIN MOD 2.0]`. Pending in-game validation for Brita/Arsenal priming and suppressor mounting. `version=0.12.0`.
- [x] LH-23 — Relook Brita de 10 rôles (armes + armure tactique). Files: `LastHomeRoles.lua`, `LastHomeShared.lua`, `README.md`, `project-state.md`, `specs/LH-23-relook-brita-10-roles.md`. Rewrote the 10 selected role blocks with their validated Brita/Arsenal loadouts (weapons, magazines/ammo, tactical clothing, D3M/X_Vest/Tactical_Alice rigs, NV for `eclaireur`), preserved the LH-22 screwdriver rule, and enabled `setUnlimitedCarry` for `rambo` and `samourai`. Used `Base.TrapMouse` and `Base.PillsVitamins` as the valid runtime IDs for the spec's uncertain `MouseTrap` / `PillsVitamin`. Pending in-game validation for priming, attachments, and worn-slot compatibility.

## Backlog

### High priority
- [ ] LH-25 — Mod séparé : choix de rôle seul (role picker standalone). Spec : `specs/LH-25-mod-roles-seul.md`. Nouveau mod standalone (`id=LastHomeRoles`) qui extrait le sous-système de rôles de Last Home : picker + application de loadout, SANS maison/TP/vagues/stock/confinement/bootstrap. Set hybride : rôles vanilla par défaut + rôles Brita (LH-22 agent + LH-23 relook) si Brita/Arsenal détectés au runtime (`getScriptManager():getItem("Base.PPK")`). Builder refill 10 min retiré ; `setUnlimitedCarry` conservé. Code dupliqué (pas de `require` vers last-home). Décisions validées : mod séparé, spawn vanilla, vanilla+Brita si détecté. Questions ouvertes : nom/ID final, API détection Brita à valider en B41, namespace `modData` propre (`LR_role`) vs `LH_role`.
- [ ] LH-24 — Déclenchement manuel de la 1ère vague (pas de timer d'auto-start). Spec : `specs/LH-24-declenchement-manuel-vague-1.md`. Décisions validées : touche `K` existante, n'importe quel joueur vivant (gate vivant ajouté à `skipToNextWave`), pas de fallback. Changements : `startPrepPhase` n'arme pas de `phaseEndsAt` pour la vague 1 ; `updatePhaseState` auto-start gate `Server.currentWave > 0` ; HUD vague 1 affiche « Appuyez sur K pour lancer la vague 1 » au lieu du compte à rebours. Le reste (vague 1 = 5 min, preps suivantes = 5 min auto-start, skip K) inchangé.
